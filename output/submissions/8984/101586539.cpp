...
map<lint, vector<lint>> mpUp; map<lint, vector<lint>> mpDown;
...
return a < b;
    ...
    for(int now = 0; now < n; now++){
        sort(mpUp[stick[now].first].begin(),mpUp[stick[now].first].end(), [](auto a, auto b){
            return a > b;
});
...
break;
        ...
        sort(mpDown[stick[now].second].begin(),mpDown[stick[now].second].end(), [](auto a, auto b){
            return a > b;
});

...
int main(){
    ...
    for (int i=0; i<N; i++){
        ...
        // 둘레 좌표를 정점 인덱스로 변환하는 작업 필요

        // s_pos, e_pos와 dist 배열 이용해서 해당 점이 속한 에지 양 끝 정점 인덱스 찾기

        // 여기서는 구현 단순화를 위해 s_idx, e_idx라 가정 (실제론 이진 탐색 필요)

        int s_idx=1, e_idx=2; // 가상의 값

        // 실제 구현에서는 s_pos, e_pos를 이용해 dist 배열을 이진 탐색하여

        // s_pos, e_pos가 어떤 정점 사이에 위치하는지 찾고 s_idx, e_idx 결정

        // 짧은 경로 방향 결정

        // 여기서는 perimeter=dist[P], d1 = (e_pos - s_pos + perimeter)%perimeter 등 계산

        // 실제로는 s_pos, e_pos 제대로 계산 후 진행

        bool forward=true; // 가정

        if (forward) {

            // s_idx에서 e_idx로 가는 경로 상의 정점들 +1

            // 인덱스 변환에서 주의 필요. 여기서는 단순 가정

            if (s_idx<=e_idx) incRange(s_idx,e_idx);

            else incRange(s_idx,e_idx+P);

        } else {

            // 반대 방향

            if (e_idx<=s_idx) incRange(e_idx,s_idx);

            else incRange(e_idx,s_idx+P);

        }

    }

    // Fenwicksum에서 각 정점별 값 추출

    // dist 배열로 정점 인덱스: 1-based fenw사용 가정

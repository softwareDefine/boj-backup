...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cin >> d >> len;
        if (d == 'R') {
            for (int k = 0; k < len; k++) H[y][x + k] = 1;
            x += len;
        } else if (d == 'L') {
            for (int k = 1; k <= len; k++) H[y][x - k] = 1;
            x -= len;
        } else if (d == 'U') {
            for (int k = 0; k < len; k++) V_[x][y + k] = 1;
            y += len;
        } else if (d == 'D') {
            for (int k = 1; k <= len; k++) V_[x][y - k] = 1;
            y -= len;
        }
    }
    ...
    bool foundAny = false, foundOneSide = false;
    long long bestAreaAll = (1LL<<60), bestAreaOne = (1LL<<60);
    int axAll=0, ayAll=0, bxAll=0, byAll=0;
    int axOne=0, ayOne=0, bxOne=0, byOne=0;
        ...
        for (int y2 = y1 + 1; y2 <= 100; y2++) {
            ...
            for (int x1 = 1; x1 <= 99; x1++) {
                for (int x2 = x1 + 1; x2 <= 100; x2++) {
                    ...
                    if (interiorH != 0 || interiorV != 0) continue;
                    ...
                    if (w == 1 || h == 1) {
                        if (area < bestAreaOne) {
                            bestAreaOne = area;
                            axOne = x1; ayOne = y1; bxOne = x2; byOne = y2;
                            foundOneSide = true;
                        }
                    }
                    if (area < bestAreaAll) {
                        bestAreaAll = area;
                        axAll = x1; ayAll = y1; bxAll = x2; byAll = y2;
                    }
                }
            }
        }

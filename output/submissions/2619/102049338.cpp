...
lint xbound[2200][2200]; lint ybound[2200][2200];
lint visited[2200][2200];
...
lint bfs(lint i, lint j){
    while(!qu.empty()){
        qu.pop();
    }
    lint cnt = 0, mmnx = 99999999, mmxx = -10000000, mmny = 99999999, mmxy = -10000000, yable = 0, xable = 0;
    qu.push({i, j});
    visited[i][j] = 1;
    while(!qu.empty()){
        auto [x, y] = qu.front();
        cnt += 1;
        if(ybound[x+1][y]){ yable = 1; }
        if(xbound[x][y+1]){ xable = 1; }
    ...
    return (((mmxx - mmnx + 1) * (mmxy - mmny + 1) == cnt) && yable && xable);
}
...
int main(){
    ...
    for(int p = 0; p < n; p++){
        auto now = lines[p], next = lines[(p+1)%n];
        if(now.x == next.x){
            if(now.y > next.y){

...
int xbound[2000][2000]; int ybound[2000][2000];
...
int bfs(int i, int j){
    queue<point> qu;
    int cnt = 0, mmnx = 9999, mmxx = 0, mmny = 9999, mmxy = 0;  
    qu.push({i, j});
    visited[i][j] = 1;
    while(!qu.empty()){
        auto [x, y] = qu.front();
        ...
        if(x+1 <= 1000 && !ybound[x+1][y] && !visited[x+1][y]){
            visited[x+1][y] = 1;
            qu.push({x+1, y});
        }
        ...
        if(y+1 <= 1000 && !xbound[x][y+1] && !visited[x][y+1]){
            visited[x][y+1] = 1;
            qu.push({x, y+1});
        }
    }
...
int main(){
    ...
    for(auto &[a, b] : lines){
        ...
        b = lower_bound(ycoor.begin(), ycoor.end(), b) - ycoor.begin();
    }
    for(int p = 0; p < n-1; p++){

...
class segtree{
    public:
        ...
        range query(int start,int end,int left,int right,int node){
            ...
            return {max(max(node1.all,node2.all),node1.right+node2.left),max(node1.left,sum(1,6000,start,(start+end)/2,1) + node2.left),max(node2.right,sum(1,6000,(start+end)/2+1,end,1) + node1.right)};
        }
        range update(int start,int end,int pos,lint val,int node){
            ...
            return tree[node] = {max(max(node1.all,node2.all),node1.right+node2.left),max(node1.left,sum(1,6000,start,(start+end)/2,1) + node2.left),max(node2.right,sum(1,6000,(start+end)/2+1,end,1) + node1.right)};
        }
...
int main(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=i;j<n;j++){
            seg.updatesum(1,6000,arr[j].first.first,arr[j].second,1);
            seg.update(1,6000,arr[j].first.first,arr[j].second,1);
            
            if(arr[j].first.second != arr[j+1].first.second){
                mmx = max(mmx,seg.query(1,6000,1,6000,1).all);
            }
        }

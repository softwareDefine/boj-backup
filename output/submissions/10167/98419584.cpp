...
class segtree{
    public:
        ...
        range update(int start,int end,int pos,lint val,int node){
            ...
            return tree[node] = {max(max(node1.all,node2.all),node1.right+node2.left),max(node1.left,sum(0,10000,start,(start+end)/2,1) + node2.left),max(node2.right,sum(0,30000,(start+end)/2+1,end,1) + node1.right)};
        }

...
int main(){
    ...
    while(t--){
        ...
        for(int i=1;i<=n;i++){
            ...
            dp[1][i][0] = max(dp[1][i-1][1],max(dp[1][i-1][0],max(dp[2][i-1][1],max(dp[2][i][0],dp[2][i-1][0]))));
            dp[2][i][0] = max(dp[2][i-1][1],max(dp[2][i-1][0],max(dp[1][i-1][1],max(dp[1][i][0],dp[1][i-1][0]))));
        }

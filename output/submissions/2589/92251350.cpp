...
int go(int x,int y){
    ...
    while(!qu.empty()){
        ...
        locmmx = max(locmmx,ncost);
        qu.pop();
        if(nx+1 < l && mp[nx+1][ny] == 'L' && !cost[nx+1][ny]){
           cost[nx+1][ny] =  ncost+1;
           qu.push({{nx+1,ny},ncost+1});
        }
        if(nx-1 >= 0 && mp[nx-1][ny] == 'L' && !cost[nx-1][ny]){
            cost[nx-1][ny] =  ncost+1;
            qu.push({{nx-1,ny},ncost+1});
        }
        if(ny+1 < w && mp[nx][ny+1] == 'L' && !cost[nx][ny+1]){
            cost[nx][ny+1] =  ncost+1;
            qu.push({{nx,ny+1},ncost+1});
        }
        if(ny-1 >= 0 && mp[nx][ny-1] == 'L' && !cost[nx][ny-1]){
            cost[nx][ny-1] =  ncost+1;
            qu.push({{nx,ny-1},ncost+1});
        }
    }

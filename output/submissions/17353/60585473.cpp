...
public:
	ll tree[1 << 22];
	ll arr[1000100];
	pair<ll,ll> lazy[1 << 22];
	void lazy_update(ll start,ll end,ll node){
		if (lazy[node].second) {
			tree[node] += lazy[node].first * (end - start + 1);
			tree[node] += lazy[node].second * (end - start+1)* (end - start + 2) / 2;
			ll mid = (start + end) / 2;
			if (start != end) {
				lazy[node * 2].first += lazy[node].first;
				lazy[node * 2].second += lazy[node].second;
				lazy[node * 2 + 1].first += lazy[node].first + mid-start+1;
	...
	void update(ll start, ll end, ll left, ll right, ll node) {
		lazy_update(start, end, node);
		if (end < left || right < start) {
			return;
		}
		if (left <= start && end <= right) {
			lazy[node].first += start-left;
			lazy[node].second += 1;
			lazy_update(start, end, node);
			return;
		}

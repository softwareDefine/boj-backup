...
int main()
{
	...
	if (cross(l1, l2)) {
		...
		if (!(isinf(a1) && isinf(a2)) && (isinf(a1) || isinf(a2))) {
			if (isinf(a1)) {
				printf("%.20Lf %.20Lf", (long double)l1.first.x, a2 * l1.first.x + b2);
			}
			else {
				printf("%.20Lf %.20Lf", (long double)l2.first.x, a1 * l2.first.x + b1);
			}
		}
		else {
			if (isnan(k) || isnan(y1)) {
				...
				if (l1.first == l2.first || l2.second==l1.second) {
				}
				else {
					if (l1.first == l2.second) {
						printf("%Lf %Lf", (long double)l1.first.x, (long double)l1.first.y);
					}
					else if (l1.second == l2.first) {
						printf("%Lf %Lf", (long double)l1.second.x, (long double)l1.second.y);					}
				}
			}
			else {

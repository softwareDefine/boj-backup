...
int main(){
    ...
    dq.push_back({startPoint.first, startPoint.second, (mp[startPoint.first][startPoint.second]?k:1)});
    while(!dq.empty()){
        auto [i, j, cnt] = dq.front();
        ...
        if(i-1 > 0 && cost[i-1][j] > cnt + (mp[i-1][j]?k:1)){
            if(mp[i-1][j] == 1){
                dq.push_back({i-1, j, cnt + k});
            }else{
                dq.push_front({i-1, j, cnt + 1});
            }
        ...
        if(j-1 > 0 && cost[i][j-1] > cnt + (mp[i][j-1]?k:1)){
            if(mp[i][j-1] == 1){
                dq.push_back({i, j-1, cnt + k});
            }else{
                dq.push_front({i, j-1, cnt + 1});
            }

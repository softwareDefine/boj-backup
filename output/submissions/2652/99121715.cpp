...
bool fitter(int i,int j) {
	...
	qu3.push({i,j});
	while(!qu3.empty()) {
		int i = qu3.front().first;
		int j = qu3.front().second;
		qu3.pop();
		chhk++;
		if(i+1<=n && chkmp[i+1][j] == 1 && !(visited3[i+1][j])) {
			visited3[i+1][j] = 1;
			qu3.push({i+1,j});
		}
		if(i-1 >= 1 && chkmp[i-1][j] == 1 && !(visited3[i-1][j])) {
			visited3[i-1][j] = 1;
			qu3.push({i-1,j});
		}
		if(j+1<=n && chkmp[i][j+1] == 1 && !(visited3[i][j+1])) {
			visited3[i][j+1] = 1;
			qu3.push({i,j+1});
		}
		if(j-1>=1 && chkmp[i][j-1] == 1 && !(visited3[i][j-1])) {
			visited3[i][j-1] = 1;
			qu3.push({i,j-1});
		}
	}

...
long long create(int ni, int ns, int ne) {
	...
	int nm = (ns + ne) / 2;
	return seg[ni] = create(ni * 2, ns, nm) * create(ni * 2 + 1, nm + 1, ne) % 1000000007;
}
void update(int ni, int ns, int ne, int qi, long long qx) {
	...
	seg[ni] = seg[ni * 2] * seg[ni * 2 + 1] % 1000000007;
}
long long query(int ni, int ns, int ne, int qs, int qe) {
	...
	int nm = (ns + ne) / 2;
	return  query(ni * 2, ns, nm, qs, qe) * query(ni * 2 + 1, nm + 1, ne, qs, qe) % 1000000007;
}
int main()
{
	...
	for (long long i = 1; i <= m + k; i++) {
		...
		if (a == 1) {
			...
			cout << query(1, 1, n, b, c) % 1000000007 << "\n";
		}
	}
}
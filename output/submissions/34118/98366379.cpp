...
lint go(int korea,int koi) {
	if(dp[korea][koi] != 9999999999999999) {
		return dp[korea][koi];
	}
	if(korea > n && koi > n) {
		return 0;
	...
	if(korea > n){
    	for(int i=0;i<=0; i++) {
    		for(int j=1; j<=1; j++) {
    			dp[korea][koi] = min(dp[korea][koi],go(korea+i,koi+j)+ brr[koi]*p1);
    		}
    	}
    	for(int i=0; i<=0; i++) {
    		for(int j=1; j<=3; j++) {
    			dp[korea][koi] = min(dp[korea][koi],go(korea+i,koi+j) + p3);
    		}
    	}
    	for(int i=0; i<=0; i++) {
    		for(int j=1; j<=5; j++) {
    			dp[korea][koi] = min(dp[korea][koi],go(korea+i,koi+j) + p5);
    		}
    	}
    	for(int i=0; i<=0; i++) {
    		for(int j=1; j<=4; j++) {
    			dp[korea][koi] = min(dp[korea][koi],go(korea+i,koi+j)+ ppair );
    		}
    	}
    	return dp[korea][koi];
	}
	if(koi > n){
    	for(int i=1; i<=1; i++) {
    		for(int j=0; j<=0; j++) {
    			dp[korea][koi] = min(dp[korea][koi],go(korea+i,koi+j)+ arr[korea]*p1);
    		}
    	}
    	for(int i=1; i<=3; i++) {
    		for(int j=0; j<=0; j++) {

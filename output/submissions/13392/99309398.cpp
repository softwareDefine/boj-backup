...
void bfs(int sn,int sR) {
	...
	while(!qu.empty()) {
		...
		if(current > future) {
			if(dp[(now+1)%2][Rightspin+future+10 - current] > dp[now%2][Rightspin] + future+10 - current) {
				dp[(now+1)%2][Rightspin+future+10 - current] = dp[now%2][Rightspin] + future+10 - current;
				qu.push({now+1,Rightspin + future+10-current});
			}
			...
			if(dp[(now+1)%2][Rightspin+future-current] > dp[now%2][Rightspin] + future - current) {
				dp[(now+1)%2][Rightspin+future-current] = dp[now%2][Rightspin] + future - current;
				qu.push({now+1,Rightspin+future-current});
			}
			dp[now%2][Rightspin] = 999999999;
		}
		if(now == 0){
